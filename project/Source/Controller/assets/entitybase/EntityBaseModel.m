...
@synthesize entityBaseSearchList = _entityBaseSearchList;
...
{
    if (self = [super init]) {
        self.entityBaseSearchList = [[NSMutableArray alloc] init];
    }
...
- (void) dealloc {
    TT_RELEASE_SAFELY(_entityBaseSearchList);
    [super dealloc];
}

- (void)load:(TTURLRequestCachePolicy)cachePolicy more:(BOOL)more {
    AppDelegate *delegate=(AppDelegate*)[[UIApplication sharedApplication] delegate];
    NSString *server_base = [NSString stringWithFormat:@"%@/assets/entitybase!getEntityBaseListByBaseParam.action", delegate.SERVER_HOST];
    TTURLRequest* request = [TTURLRequest requestWithURL: server_base delegate: self];
    [request setHttpMethod:@"POST"];
    
    request.contentType=@"application/x-www-form-urlencoded";
    NSString* postBodyString = [NSString stringWithFormat:@"isMobile=true&page=1&pageSize=%i%@",self.pageSize,self.searchQuery];
...
- (void)requestDidFinishLoad:(TTURLRequest*)request {
    ...
    if (!success) {
        ...
        EntityBase *entityBase = [[EntityBase alloc] init];
        _entityBaseSearchList = [entityBase initEntityBase:[jsonDic objectForKey:@"entityBaseList"]];
        TT_RELEASE_SAFELY(entityBase);
    }
